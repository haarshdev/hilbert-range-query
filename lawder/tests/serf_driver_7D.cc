...
//   [--vec_already_ms] (do not multiply Vec/Height/Adjustment by 1000)
//   [--json <path>]
//   [--fp_counts_json <path>]
...
//
// NEW (this update):
//   Move from 6D -> 7D by adding Height and Adjustment as two separate dimensions:
//     dim0..4 = Vec[0..4] in ms
//     dim5    = Height in ms
//     dim6    = Adjustment in ms
//   The index and the sphere cover now operate in 7D.
...
static string point_key(const array<PU_int,DIMS>& p) {
    std::string out;
...
static inline bool isSingleCell(const array<long long,DIMS>& lo,
                                const array<long long,DIMS>& hi)
{
    for (int d=0; d<DIMS; d++) if (lo[d] != hi[d]) return false;
    return true;
}

static inline long long dist2_PointToPoint(
    const array<long long,DIMS>& a,
    const array<long long,DIMS>& b)
{
    long long acc = 0;
    for (int d=0; d<DIMS; d++) {
        long long t = a[d] - b[d];
...
static inline double minDist2_PointToBox_ms(
    const array<double,DIMS>& c,
    const array<double,DIMS>& lo,
    const array<double,DIMS>& hi)
{
    double acc = 0.0;
    for (int d=0; d<DIMS; d++) {
        double x = c[d];
...
static bool box_has_any_point(DBASE* DB, const array<long long,DIMS>& lo, const array<long long,DIMS>& hi) {
    PU_int LB[DIMS], UB[DIMS], result[DIMS];
    ...
    QueryStats& qs)
{
    ...
    if (true == DB->db_range_open_set(LB, UB, &set_id)) {
        ...
        while (true == DB->db_range_fetch_another(set_id, result)) {
            ...
            array<PU_int,DIMS> rp;
            for (int d=0; d<DIMS; d++) rp[d] = result[d];
...
static void coverSphere7D_indexed(
    DBASE* DB,
    int ORDER,
    const array<double,DIMS>& center_ms,  // center in Step-2 shifted ms-space (7D)
    ...
    QueryStats& qs)
{
    ...
    // Subdivide into 128 children (7D)
    array<long long,DIMS> mid;
    for (int d=0; d<DIMS; d++) mid[d] = (lo[d] + hi[d]) >> 1;

    for (int mask=0; mask<(1<<DIMS); mask++) {
        array<long long,DIMS> clo, chi;
        for (int d=0; d<DIMS; d++) {
        ...
        bool empty=false;
        for (int d=0; d<DIMS; d++) if (clo[d] > chi[d]) { empty=true; break; }
        if (empty) continue;

        coverSphere7D_indexed(DB, ORDER, center_ms, T_ms, cell_size_ms,
...
int main(int argc, char** argv) {
    ...
    // output CSVs
    std::string out_summary_csv = "tableA_summary.csv";
    std::string out_nodes_csv   = "tableB_nodes.csv";
    ...
    const double SCALE_TO_MS = vec_already_ms ? 1.0 : 1000.0;

    // ---------- extract all names + 7D coordinates ----------
    vector<string> names(N);
    vector<array<double,DIMS>> coords_ms(N); // [Vec0..Vec4, Height, Adjustment]
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        for (int d = 0; d < 5; d++) coords_ms[i][d] = v[d].get<double>() * SCALE_TO_MS;

        double h = 0.0, adj = 0.0;
        if (c.contains("Height")) h = c["Height"].get<double>() * SCALE_TO_MS;
        if (c.contains("Adjustment")) adj = c["Adjustment"].get<double>() * SCALE_TO_MS;
    ...
    // Use a different DB name so 7D does not collide with old DB files
    std::string dbname = "serfdb_7d_o" + std::to_string(ORDER);
    ...
    for (size_t i=0; i<N; i++) {
        for (int d=0; d<DIMS; d++) {
            mn[d] = std::min(mn[d], coords_ms[i][d]);
            mx[d] = std::max(mx[d], coords_ms[i][d]);
        }
    }

    if (debug) {
        cout << "Shift minima mn[d] (ms): ";
        for (int d=0; d<DIMS; d++) cout << mn[d] << (d==DIMS-1? "\n":" ");
        cout << "Shift maxima mx[d] (ms): ";
        for (int d=0; d<DIMS; d++) cout << mx[d] << (d==DIMS-1? "\n":" ");
        cout << "Scaling mode: SHIFT-ONLY (no global-span normalization)\n";
        cout << "Extra dims: Height (dim5), Adjustment (dim6)\n";
    }
    ...
    vector<array<PU_int,DIMS>> pts(N);
    for (size_t i=0; i<N; i++) {
        for (int d=0; d<DIMS; d++) pts[i][d] = quantize_prof(coords_ms[i][d], d);
    }

    cout << "Query point grid coords: ";
    for (int d=0; d<DIMS; d++) cout << pts[qi][d] << (d==DIMS-1? "\n":" ");
    ...
    if (rebuild || !db_exists) {
        ...
        for (size_t i=0; i<N; i++) {
            PU_int p[DIMS];
            for (int d=0; d<DIMS; d++) p[d] = pts[i][d];
    ...
    array<long long,DIMS> center;
    for (int d=0; d<DIMS; d++) center[d] = (long long)pts[qi][d];

    array<long long,DIMS> root_lo, root_hi;
    for (int d=0; d<DIMS; d++) {
    ...
    cout << "  LB:";
    for (int d=0; d<DIMS; d++) cout << " " << root_lo[d];
    cout << "\n";
    cout << "  UB:";
    for (int d=0; d<DIMS; d++) cout << " " << root_hi[d];
    cout << "\n";

    array<double,DIMS> center_ms;
    for (int d=0; d<DIMS; d++) {
        double s = coords_ms[qi][d] - mn[d];
        if (s < 0.0) s = 0.0;
    ...
    // Validation vs brute-force sphere in grid units (now 7D)
    ...
    for (size_t i=0; i<N; i++) {
        ...
        if (nm == qname) continue;
        array<long long,DIMS> p;
        for (int d=0; d<DIMS; d++) p[d] = (long long)pts[i][d];
    ...
    // Build node lists for Table B (details)
    ...
    // Append Table B nodes CSV (short names)
    {
