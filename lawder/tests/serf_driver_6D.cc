...
//
// NEW (this update):
//   Move from 5D -> 6D by adding one extra dimension:
//     dim5 = (Height + Adjustment) in ms
//   The index and the sphere cover now operate in 6D.
...
static void coverSphere6D_indexed(
    DBASE* DB,
    int ORDER,
    const array<double,DIMS>& center_ms,  // center in Step-2 shifted ms-space (6D)
    ...
    QueryStats& qs)
{
    ...
    for (int mask=0; mask<(1<<DIMS); mask++) {
        ...
        coverSphere6D_indexed(DB, ORDER, center_ms, T_ms, cell_size_ms,
...
int main(int argc, char** argv) {
    ...
    // ---------- extract all names + 6D coordinates ----------
    vector<string> names(N);
    vector<array<double,DIMS>> coords_ms(N); // [Vec0..Vec4, extra=(H+Adj)]
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        for (int d = 0; d < 5; d++) coords_ms[i][d] = v[d].get<double>() * VEC_TO_MS;

        double h = 0.0, adj = 0.0;
        if (c.contains("Height")) h = c["Height"].get<double>() * VEC_TO_MS;
        if (c.contains("Adjustment")) adj = c["Adjustment"].get<double>() * VEC_TO_MS;

        coords_ms[i][5] = h + adj; // the new 6th dimension
    }
    ...
    // Use a different DB name so 6D does not collide with old 5D DB files
    std::string dbname = "serfdb_6d_o" + std::to_string(ORDER);
    ...
    if (debug) {
        ...
        cout << "Extra dim: Height+Adjustment (ms)\n";
    }
    ...
    if (rebuild || !db_exists) {
        int inserted = 0;
        for (size_t i=0; i<N; i++) {
            PU_int p[DIMS]; for (int d=0; d<DIMS; d++) p[d] = pts[i][d];
    ...
    // Validation vs brute-force sphere in grid units (now 6D)
